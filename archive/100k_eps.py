...
class federated_matrix_factorization_with_projection_p:
    ...
    def update_U(self, sample_users):
        for i in sample_users:
            ...
            self.__U[i] = spsolve((VT_C_V + self.__lambda * sparse.eye(self.__latent_factor)), sparse.csr_matrix.toarray(VT_C_Rui)).flatten()
    ...
    def loss(self):
        ...
        loss = cu.sum(self.__C_uncertain*(r_pre- self.__rating_train)**2)+ self.__lambda* (cu.linalg.norm(self.__U)**2)+self.__miu* (cu.linalg.norm(self.__V)**2)
        return loss
...
# data preprocessing
rating_matrix = dataset_loader("Movie", "/mnt/sda/MF_secagg/ml-100k/ml-100k/u.data")
...
k = 14
# privacy budgets
print("eps = 0.01______________________________________________________________________")
pb_eps = 0.01
pb_alpha = 2
p = item_size//2
...
# privacy budgets
print("eps = 0.05______________________________________________________________________")
pb_eps = 0.05
pb_alpha = 2
p = item_size//2
pbm_m, theta = get_theta_m(pb_eps, pb_alpha, 1, user_size, p, k)
...
# privacy budgets
print("eps = 0.5______________________________________________________________________")
pb_eps = 0.5
pb_alpha = 2
p = item_size//2
pbm_m, theta = get_theta_m(pb_eps, pb_alpha, 1, user_size, p, k)
