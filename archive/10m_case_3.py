...
def dataset_loader(
    ...
    return cu.asarray(rating_matrix.iloc[:n,:m].values)
    ...
    def train(self, B_1, theta, m_pbm, maxitr = 20, sample_pct = 0.1):
        ...
        for i in range(maxitr):
            ...
            for j in range(sample_size):
                ...
                flat_gradient_V_j = self.flatMa(gradient_V_j) # flatten

                # PBM
                flat_gradient_V_j = cu.clip(flat_gradient_V_j, -c_1, c_1)
                Rescale_flat_gradient_V_j = (theta/c_1) * flat_gradient_V_j + 0.5
                Z_j = cu.random.binomial(m_pbm, Rescale_flat_gradient_V_j)
                Z_sum  = Z_sum + Z_j

            gradient_V = c_1 / (c_2 * theta) * (Z_sum - c_2 / 2) # SecAgg
            gradient_V = gradient_V / sample_pct # rescale
            gradient_V = self.RevMat(gradient_V) # unflatten

            # ADAM
            m_adam = self.__beta_1*m_adam + (1-self.__beta_1)* gradient_V
            m_adam_adjust = m_adam/(1-self.__beta_1)
            v_adam = self.__beta_2*v_adam + (1-self.__beta_2)* gradient_V * gradient_V
            v_adam_adjust = v_adam/(1-self.__beta_2)
            self.__V = self.__V - ((maxitr - i)/maxitr)*self.__lr_adam*m_adam_adjust/(cu.sqrt(v_adam_adjust) + self.__epsilon_adam)
...
# data preprocessing
rating_matrix = dataset_loader("Movie", "/home/FL_repo/FL_SecAgg/10mdataset/ratings.dat")
...
p_2 = 2 ** math.ceil(math.log2(item_size))
...
mf = federated_matrix_factorization_with_projection_p(
    train_data = train_val_matrix, rating_matrix = rating_matrix, test_index_matrix = test_index_matrix, k = k, p = item_size, p_2 = p_2,
    e_max = 2, e_min = -1.5, user_size = user_size, item_size = item_size,
...
mf.train(B_1 = 0.001, theta = 0.25, m_pbm = 1, maxitr = 200, sample_pct = 0.1) # without calculating m_pbm --> m_pbm = 1, without calculating theta --> theta = 0.25
