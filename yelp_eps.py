...
class federated_matrix_factorization_with_projection_p:
    ...
    def train(self, B_1, theta, m_pbm, maxitr = 20, sample_pct = 0.1):
        ...
        for i in range(maxitr):
            ...
            t1_2 = time.time()
            t  = t + t1_2 - t1_1
            print(f'Iteration time: {t1_2 - t1_1}')
...
rating_matrix = pd.pivot_table(rating_df,values = ['Rating'],index=['UserID'], columns=['ItemID'],fill_value=0)
rating_matrix = cu.asarray(rating_matrix.values)
...
mf.train(B_1 = 0.001, theta = theta, m_pbm = pbm_m, maxitr = 1, sample_pct = 0.1) # without calculating m_pbm --> m_pbm = 1, without calculating theta --> theta = 0.25